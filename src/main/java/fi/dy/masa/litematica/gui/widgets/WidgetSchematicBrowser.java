...
public class WidgetSchematicBrowser extends WidgetFileBrowserBase {
    ...
    public WidgetSchematicBrowser(int x, int y, int width, int height, GuiSchematicBrowserBase parent, @Nullable ISelectionListener<DirectoryEntry> selectionListener) {
        super(x, y, width, height, DataManager.getDirectoryCache(), parent.getBrowserContext(), parent.getDefaultDirectory(), selectionListener, Icons.FILE_ICON_LITEMATIC);
    ...
    @Override
    protected void drawAdditionalContents(int mouseX, int mouseY, GuiGraphics drawContext) {
        this.drawSelectedSchematicInfo(this.getLastSelectedEntry(), drawContext);
    }

    protected void drawSelectedSchematicInfo(@Nullable DirectoryEntry entry, GuiGraphics drawContext) {
        ...
        if (meta != null) {
            ...
            */
            ...
            if (pair != null) {
                ...
                drawContext.blit(pair.getLeft(), x + 4, y, 0.0F, 0.0F, iconSize, iconSize, iconSize, iconSize);
            }
        }
    }
        ...
        if (previewImageData != null && previewImageData.length > 0) {
            ...
            if ((size * size) == previewImageData.length) {
                try {
                    ...
                    LitematicaRewrite.LOGGER.warn("Failed to create a preview image", e);
                }
            }
        }
    }

...
public class SchematicConversionMaps {
    private static final Object2IntOpenHashMap<String> OLD_BLOCK_NAME_TO_SHIFTED_BLOCK_ID = DataFixUtils.make(new Object2IntOpenHashMap<>(), (map) -> {
        map.defaultReturnValue(-1);
    });
    private static final Int2ObjectOpenHashMap<String> ID_META_TO_UPDATED_NAME = new Int2ObjectOpenHashMap<>();
    private static final Object2IntOpenHashMap<BlockState> BLOCKSTATE_TO_ID_META = DataFixUtils.make(new Object2IntOpenHashMap<>(), (map) -> {
        map.defaultReturnValue(-1);
    });
    ...
    public static void computeMaps() {
        ...
        for (ConversionData data : CACHED_DATA) {
            try {
                ...
                LitematicaRewrite.LOGGER.warn("addEntry(): Exception while adding blockstate conversion map entry for ID '{}' (fixed state: '{}')", data.idMeta, data.newStateString, e);
            }
        }
    ...
    private static void addIdMetaToBlockState(int idMeta, CompoundTag newStateTag, String... oldStateStrings) {
        try {
            ...
            BlockState state = NbtUtils.readBlockState(lookup, newStateTag);
            ...
            LitematicaRewrite.LOGGER.warn("addIdMetaToBlockState(): Exception while adding blockstate conversion map entry for ID '{}'", idMeta, e);
        }
    }
    ...
    private static void addOldStateToNewState(CompoundTag newStateTagIn, String... oldStateStrings) {
        try {
            ...
            LitematicaRewrite.LOGGER.warn("addOldStateToNewState(): Exception while adding new blockstate to old blockstate conversion map entry for '{}'", newStateTagIn, e);
        }
    }
    ...
    public static String updateBlockName(String oldName, int oldVersion) {
        ...
        try {
            return MC.getFixerUpper().update(References.BLOCK_NAME, new Dynamic<>(NbtOps.INSTANCE, tagStr), oldVersion, LitematicaSchematic.MINECRAFT_DATA_VERSION).getValue().getAsString();
        } catch (Exception e) {
            LitematicaRewrite.LOGGER.warn("updateBlockName: failed to update Block Name [{}], preserving original state (data may become lost)", oldName);
            return oldName;
        }
    }
    ...
    public static CompoundTag updateBlockStates(CompoundTag oldBlockState, int oldVersion) {
        try {
            return (CompoundTag) MC.getFixerUpper().update(References.BLOCK_STATE, new Dynamic<>(NbtOps.INSTANCE, oldBlockState), oldVersion, LitematicaSchematic.MINECRAFT_DATA_VERSION).getValue();
        } catch (Exception e) {
            LitematicaRewrite.LOGGER.warn("updateBlockStates: failed to update Block State [{}], preserving original state (data may become lost)", oldBlockState.contains("Name") ? oldBlockState.getString("Name") : "?");
    ...
    public static CompoundTag updateBlockEntity(CompoundTag oldBlockEntity, int oldVersion) {
        try {
            return (CompoundTag) MC.getFixerUpper().update(References.BLOCK_ENTITY, new Dynamic<>(NbtOps.INSTANCE, oldBlockEntity), oldVersion, LitematicaSchematic.MINECRAFT_DATA_VERSION).getValue();
        } catch (Exception e) {
            BlockPos pos = NBTUtils.readBlockPos(oldBlockEntity);
            LitematicaRewrite.LOGGER.warn("updateBlockEntity: failed to update Block Entity [{}] at [{}], preserving original state (data may become lost)", oldBlockEntity.contains("id") ? oldBlockEntity.getString("id") : "?", pos != null ? pos.toShortString() : "?");
    ...
    public static CompoundTag updateEntity(CompoundTag oldEntity, int oldVersion) {
        try {
            return (CompoundTag) MC.getFixerUpper().update(References.ENTITY, new Dynamic<>(NbtOps.INSTANCE, oldEntity), oldVersion, LitematicaSchematic.MINECRAFT_DATA_VERSION).getValue();
        } catch (Exception e) {
            LitematicaRewrite.LOGGER.warn("updateEntity: failed to update Entity [{}], preserving original state (data may become lost)", oldEntity.contains("id") ? oldEntity.getString("id") : "?");
            return oldEntity;
        }
    }
    ...
    public static CompoundTag checkForIdTag(CompoundTag tags) {
        ...
        if (tags.contains("Items")) {
            ListTag items = fixItemsTag(tags.getList("Items", Constants.NBT.TAG_COMPOUND));
                ...
                if (tag == null) {
                    ...
                    if (tag.contains("BlockEntityTag", Constants.NBT.TAG_COMPOUND)) {
                        ...
                        if (entityEntry.contains("Items", Constants.NBT.TAG_LIST)) {
                            ListTag nestedItems = fixItemsTag(entityEntry.getList("Items", Constants.NBT.TAG_COMPOUND));

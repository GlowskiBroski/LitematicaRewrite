...
public class SchematicConversionMaps {
    ...
    public static String updateBlockName(String oldName, int oldVersion) {
        ...
        try {
            return MC.getDataFixer().update(TypeReferences.BLOCK_NAME, new Dynamic<>(NbtOps.INSTANCE, tagStr), oldVersion, LitematicaSchematic.MINECRAFT_DATA_VERSION).getValue().asString();
        } catch (Exception e) {
    ...
    public static NbtCompound updateBlockStates(NbtCompound oldBlockState, int oldVersion) {
        try {
            return (NbtCompound) MC.getDataFixer().update(TypeReferences.BLOCK_STATE, new Dynamic<>(NbtOps.INSTANCE, oldBlockState), oldVersion, LitematicaSchematic.MINECRAFT_DATA_VERSION).getValue();
        } catch (Exception e) {
            Litematica.LOGGER.warn("updateBlockStates: failed to update Block State [{}], preserving original state (data may become lost)", oldBlockState.contains("Name") ? oldBlockState.getString("Name") : "?");
    ...
    public static NbtCompound updateBlockEntity(NbtCompound oldBlockEntity, int oldVersion) {
        try {
            return (NbtCompound) MC.getDataFixer().update(TypeReferences.BLOCK_ENTITY, new Dynamic<>(NbtOps.INSTANCE, oldBlockEntity), oldVersion, LitematicaSchematic.MINECRAFT_DATA_VERSION).getValue();
        } catch (Exception e) {
            BlockPos pos = NBTUtils.readBlockPos(oldBlockEntity);
            Litematica.LOGGER.warn("updateBlockEntity: failed to update Block Entity [{}] at [{}], preserving original state (data may become lost)", oldBlockEntity.contains("id") ? oldBlockEntity.getString("id") : "?", pos != null ? pos.toShortString() : "?");
    ...
    public static NbtCompound updateEntity(NbtCompound oldEntity, int oldVersion) {
        try {
            return (NbtCompound) MC.getDataFixer().update(TypeReferences.ENTITY, new Dynamic<>(NbtOps.INSTANCE, oldEntity), oldVersion, LitematicaSchematic.MINECRAFT_DATA_VERSION).getValue();
        } catch (Exception e) {
            Litematica.LOGGER.warn("updateEntity: failed to update Entity [{}], preserving original state (data may become lost)", oldEntity.contains("id") ? oldEntity.getString("id") : "?");
            return oldEntity;
        }
    }

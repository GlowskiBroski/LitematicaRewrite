...
public class TaskPasteSchematicPerChunkCommand extends TaskPasteSchematicPerChunkBase {
    ...
    protected static boolean preparePickedStack(BlockPos pos, BlockState state, BlockEntity be, World world, MinecraftClient mc, @Nonnull DynamicRegistryManager registryManager) {
    ...
    protected void prepareSettingBlocks(IntBoundingBox box) {
        if (this.useFillCommand) {
            ...
            this.positionIterator = BlockPos.iterate(box.minX, box.minY, box.minZ, box.maxX, box.maxY, box.maxZ).iterator();
        }
    ...
    protected void processBlocksInCurrentBoxUsingSetBlockOnly() {
        ...
        Chunk clientChunk = MC.world.getChunk(chunkPos.x, chunkPos.z);
        ...
        if (this.shouldSetBlock(stateSchematic, stateClient)) {
            ...
            if (be != null && nbtBehavior != PasteNbtBehavior.NONE) {
                ...
                if (nbtBehavior == PasteNbtBehavior.PLACE_MODIFY) {
                    this.setDataViaDataModify(pos, stateSchematic, be, schematicWorld, MC.world, commandHandler);
                } else if (nbtBehavior == PasteNbtBehavior.PLACE_CLONE) {
                    this.placeBlockViaClone(pos, stateSchematic, be, schematicWorld, MC.world, commandHandler);
                }
            } else {
    ...
    protected boolean shouldSetBlock(BlockState stateSchematic, BlockState stateClient) {
        ...
        if ((stateSchematic.isAir() && stateClient.isAir()) || (this.changedBlockOnly && stateClient == stateSchematic)) {
            return false;
        }
    ...
    protected void queueFillCommandForBox(int minX, int minY, int minZ, int maxX, int maxY, int maxZ, BlockState state) {
        ...
        if (this.useWorldEdit) {
            ...
            String fillCommand = String.format("%s %d %d %d %d %d %d %s", cmdName, minX, minY, minZ, maxX, maxY, maxZ, blockString);
    ...
    protected void setDataViaDataModify(BlockPos pos, BlockState state, BlockEntity be, World schematicWorld, ClientWorld clientWorld, Consumer<String> commandHandler) {
    ...
    protected void specialPasteSignBlock(BlockPos pos, BlockState state, World schematicWorld, Consumer<String> commandHandler) {
        ...
        if (be instanceof SignBlockEntity signBe) {
            ...
            if (tag != null) {
                ...
                if (!signBe.getBackText().hasText(MC.player)) {
                    tag.remove("back_text");
                }

                if (!signBe.getFrontText().hasText(MC.player)) {
                ...
                String cmd = String.format("%s %d %d %d %s%s", cmdName, pos.getX(), pos.getY(), pos.getZ(), blockString, tag);
        ...
        commandHandler.accept(String.format("%s %d %d %d %s", cmdName, pos.getX(), pos.getY(), pos.getZ(), blockString));
        ++this.sentSetblockCommands;
    }

    protected void placeBlockViaClone(BlockPos pos, BlockState state, BlockEntity be, World schematicWorld, ClientWorld clientWorld, Consumer<String> commandHandler) {
        BlockPos placementPos = this.placeNbtPickedBlock(pos, state, be, schematicWorld, clientWorld);

        if (placementPos != null) {
            String command = String.format("%s %d %d %d %d %d %d %d %d %d", this.cloneCommand, placementPos.getX(), placementPos.getY(), placementPos.getZ(), placementPos.getX(), placementPos.getY(), placementPos.getZ(), pos.getX(), pos.getY(), pos.getZ());
            commandHandler.accept(command);
    ...
    @Nullable
    protected BlockPos placeNbtPickedBlock(BlockPos pos, BlockState state, BlockEntity be, @Nonnull World schematicWorld, @Nonnull ClientWorld clientWorld) {
        double reach = MC.player.getBlockInteractionRange();
        BlockPos placementPos = this.findEmptyNearbyPosition(clientWorld, MC.player.getPos(), 4, reach);

        if (placementPos != null && preparePickedStack(pos, state, be, schematicWorld, MC, clientWorld.getRegistryManager())) {
    ...
    protected void fillVolume(long encodedValue, int baseX, int baseZ, ChunkSchematic schematicChunk, Chunk clientChunk) {
    ...
    protected int getBlockStripLength(BlockPos.Mutable pos, Direction direction, int maxLength, BlockState firstState, Chunk chunk) {
        int length = 1;
                ...
                for (int z = startZ; z <= endZ; ++z) {
                    ...
                    if (length > 0) {
                        ...
                        if (!ignoreBe || !state.hasBlockEntity()) {
                            ...
                            while (nextX <= 15 && nextY <= box.maxY && nextZ <= 15 && workArr[nextX][nextY - worldMinY][nextZ] == length && chunk.getBlockState(mutablePos.set(nextX, nextY, nextZ)) == state) {
                                ++stripCount;
                ...
                for (int y = box.minY; y <= box.maxY; ++y) {
                    ...
                    if (packedSize != 0) {
                        ...
                        if (!ignoreBe || !state.hasBlockEntity()) {
                            ...
                            while (nextX <= 15 && nextY <= box.maxY && nextZ <= 15 && workArr[nextX][nextY - worldMinY][nextZ] == packedSize && chunk.getBlockState(mutablePos.set(nextX, nextY, nextZ)) == state) {
                                ++layerCount;
                                workArr[nextX][nextY - worldMinY][nextZ] = 0;
                                nextX += lcOffX;
                                nextY += lcOffY;
        ...
        for (double y = centerPos.getY() - radiusY; y <= centerPos.getY() + radiusY; ++y) {
            for (double z = centerPos.getZ() - radius; z <= centerPos.getZ() + radius; ++z) {
                for (double x = centerPos.getX() - radius; x <= centerPos.getX() + radius; ++x) {
                    ...
                    if (MathHelper.floor(MathHelper.abs((float) (centerPos.getX() - x))) < 2 && MathHelper.floor(MathHelper.abs((float) (centerPos.getZ() - z))) < 2 && y >= centerPos.getY() - 2 && y <= centerPos.getY() + 2) {
                        continue;
                    }
                    ...
                    if (this.placedPositionTimestamps.containsKey(posLong) && currentTime - this.placedPositionTimestamps.get(posLong) < timeout) {
                        continue;
                    }

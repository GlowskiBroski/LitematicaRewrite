    ...
    public void run() {
        while (this.shouldRun) {
            try {
                ...
                LitematicaRewrite.LOGGER.debug("Stopping chunk worker due to interrupt");
                return;
            } catch (Throwable throwable) {
                CrashReport crashreport = CrashReport.forThrowable(throwable, "Batching chunks");
                MC.delayCrashRaw(MC.fillReport(crashreport));
                return;
            }
        }
    }
    ...
    protected void processTask(final ChunkRenderTaskSchematic task) throws InterruptedException {
        ...
        try {
            if (task.getStatus() != ChunkRenderTaskSchematic.Status.PENDING) {
                if (!task.isFinished()) {
                    LitematicaRewrite.LOGGER.warn("Chunk render task was {} when I expected it to be pending; ignoring task", task.getStatus());
                }
        ...
        if (entity == null) {
            ...
            try {
                if (task.getStatus() != ChunkRenderTaskSchematic.Status.COMPILING) {
                    if (!task.isFinished()) {
                        LitematicaRewrite.LOGGER.warn("Chunk render task was {} when I expected it to be compiling; aborting task", task.getStatus());
                    }
            ...
            if (taskType == ChunkRenderTaskSchematic.Type.REBUILD_CHUNK) {
                ...
                if (!chunkRenderData.isBlockLayerEmpty(layer)) {
                    ...
                    BufferBuilder buffer = buffers.getBlockBufferByLayer(layer);
                    futuresList.add(this.chunkRenderDispatcher.uploadChunkBlocks(RenderType.translucent(), buffer, renderChunk, chunkRenderData, task.getDistanceSq()));
                }
            ...
            Futures.addCallback(listenablefuture, new FutureCallback<List<Object>>() {
                ...
                public void onSuccess(@Nullable List<Object> list) {
                    ...
                    label49:
                    {
                        try {
                            ...
                            if (!task.isFinished()) {
                                LitematicaRewrite.LOGGER.warn("Chunk render task was {} when I expected it to be uploading; aborting task", task.getStatus());
                            }
                        } finally {
                            task.getLock().unlock();
                        }
                ...
                public void onFailure(Throwable throwable) {
                    ...
                    if (!(throwable instanceof CancellationException) && !(throwable instanceof InterruptedException)) {
                        MC.delayCrashRaw(CrashReport.forThrowable(throwable, "Rendering Litematica chunk"));
                    }
                }

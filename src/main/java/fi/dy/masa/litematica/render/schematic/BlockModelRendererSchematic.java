...
public class BlockModelRendererSchematic {
    ...
    public boolean renderModel(BlockAndTintGetter worldIn, BakedModel modelIn, BlockState stateIn,
                               BlockPos posIn, PoseStack matrixStack,
                               VertexConsumer vertexConsumer, long rand) {
        boolean ao = Minecraft.useAmbientOcclusion() && stateIn.getLightEmission() == 0 && modelIn.useAmbientOcclusion();

        Vec3 offset = stateIn.getOffset(worldIn, posIn);
    ...
    public boolean renderModelSmooth(BlockAndTintGetter worldIn, BakedModel modelIn, BlockState stateIn, BlockPos posIn, PoseStack matrixStack,
    ...
    public boolean renderModelFlat(BlockAndTintGetter worldIn, BakedModel modelIn, BlockState stateIn,
                                   BlockPos posIn, PoseStack matrixStack,
                                   VertexConsumer vertexConsumer, BitRandomSource random, long seedIn, int overlay) {
        boolean renderedSomething = false;
        BitSet bitset = new BitSet(3);
        ...
        for (Direction side : PositionUtils.ALL_DIRECTIONS) {
            ...
            if (!quads.isEmpty()) {
                if (this.shouldRenderModelSide(worldIn, stateIn, posIn, side)) {
                    int light = LevelRenderer.getLightColor(worldIn, stateIn, posIn.relative(side));
    ...
    protected boolean shouldRenderModelSide(BlockAndTintGetter worldIn, BlockState stateIn, BlockPos posIn, Direction side) {
        return DataManager.getRenderLayerRange().isPositionAtRenderEdgeOnSide(posIn, side) ||
                ...
                Block.shouldRenderFace(stateIn, worldIn, posIn, side, posIn.relative(side));
    }

    protected void renderQuadsSmooth(BlockAndTintGetter world, BlockState state, BlockPos pos, PoseStack matrixStack,
        ...
        for (int i = 0; i < size; ++i) {
            ...
            this.getQuadDimensions(world, state, pos, bakedQuad.getVertices(), bakedQuad.getDirection(), box, flags);
            ambientOcclusionCalculator.apply(world, state, pos, bakedQuad.getDirection(), box, flags, bakedQuad.isShade());
    ...
    protected void renderQuadsFlat(BlockAndTintGetter world, BlockState state, BlockPos pos,
        ...
        for (int i = 0; i < size; ++i) {
            ...
            if (useWorldLight) {
                this.getQuadDimensions(world, state, pos, bakedQuad.getVertices(), bakedQuad.getDirection(), null, flags);
                BlockPos blockPos = flags.get(0) ? pos.relative(bakedQuad.getDirection()) : pos;
                light = LevelRenderer.getLightColor(world, state, blockPos);
            }
    ...
    protected void renderQuad(BlockAndTintGetter world, BlockState state, BlockPos pos, VertexConsumer vertexConsumer, PoseStack matrixStack,
        ...
        vertexConsumer.putBulkData(matrixStack.last(), quad, new float[]{brightness0, brightness1, brightness2, brightness3},
                r, g, b, 1.0f, new int[]{light0, light1, light2, light3}, overlay, true);
    }

    protected void getQuadDimensions(BlockAndTintGetter world, BlockState state, BlockPos pos, int[] vertexData, Direction face, @Nullable float[] box, BitSet flags) {
        float minX = 32.0F;
        float minY = 32.0F;
        float minZ = 32.0F;
        float maxX = -32.0F;
